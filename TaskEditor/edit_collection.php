...
<div class="background">
    ...
    <div class="container">
        ...
        if (count($cardArr) == 0) {
            ...
            echo "<textarea class='form-control' name='formula' rows='1'>" . substr($card['formula'], 1, -1) . "</textarea>";
            echo "<h5 class='mt-3'>Описание:</h5>";
            echo "<textarea type='text' class='form-control' name='description' rows='3'>" . $card['description'] . "</textarea>";
            echo "<h5 class='mt-3'>Пояснение:</h5>";
            echo "<textarea class='form-control' name='explanation' rows='7'>" . $card['explanation'] . "</textarea>";
...
<script>
    ...
    function addCard() {
        window.location.href = "/TaskEditor/add_card.php?podbor=<?php echo $_GET['podbor'] ?>";
    }
</script>
<script>
    function deleteCard() {
        if (confirm("Вы уверены, что хотите удалить карточку?")) {
            window.location.href = "/TaskEditor/delete_card.php?podbor=<?php echo $_GET['podbor'] ?>&card=<?php echo $card['card_id'] ?>";
        }
    }
</script>
</html>